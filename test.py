...
@bot.tree.command(name="check", description="check if a player is banned or add them to the watch list")
async def check(interaction: discord.Interaction, player: str):
    ...
    if response.status_code == 200:
        ...
        if not any(player.lower() == p.lower() for p in data):
            await interaction.response.send_message(f"{player} est banni")
        else:
            if not any(player.lower() == t.lower() for t in termes):
                termes.append(player)
                save_players()
            await interaction.response.send_message(
                f"{player} (trouvé comme '{data[0]}') n'est pas banni et a été ajouté à la liste de surveillance.")
    else:
        await interaction.response.send_message(f"Erreur lors de la requête pour {player}")
